...
void interpret_mpu(void){
    ...
    if (program_state == DATA_READY){
        ...
        if ((fabs(*az_po+1.0) > 0.05) && (fabs(*az_po+1.0) < 0.20)){
            *az_sum_po += fabs(*az_po);
        }
        if (*az_po < -1.15) {
            sprintf(move_msg, " \r\n\0");
            System_printf("FALL\n");
            System_flush();
            program_state = SENDING;
            }else if (((fabs(*ax_sum_po) + fabs(*ay_sum_po)) > 0.06) && (fabs(*az_sum_po) <= 0.02) && (fabs(*ax_prev_po - *ax_po) > 0.01) && (fabs(*ax_prev_po - *ax_po) > 0.01)){ //ax > 2.0 | ax < -2.0){ //= LIUKUU?
